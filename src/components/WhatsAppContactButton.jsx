...
const WhatsAppContactButton = ({ 
    ...
    const generateMessage = () => {
        let message = 'Olá! Tenho interesse em um produto do seu anúncio';
        ...
        if (advertisementId) {
            message += `. ID do anúncio: ${advertisementId}`;
        }
    ...
    // Se não há número, não renderiza o botão
    if (!whatsappNumber) {
        return null;
    }

    const formattedNumber = formatWhatsAppLink(whatsappNumber);
    
    if (!formattedNumber) {
        return null;
    }

    const whatsappUrl = `https://wa.me/${formattedNumber}?text=${generateMessage()}`;
