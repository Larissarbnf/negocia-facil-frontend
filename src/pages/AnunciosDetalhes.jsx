...
const AnunciosDetalhes = () => {
    ...
    // 🔧 FUNÇÃO CORRIGIDA PARA OBTER ID DO USUÁRIO ATUAL
    const getCurrentUserId = () => {
        console.log('🔍 Verificando usuário atual...');
        
        try {
            // MÉTODO PRINCIPAL: Extrair do token JWT
            const token = localStorage.getItem('token');
            if (token) {
                ...
                if (parts.length === 3) {
                    ...
                    // Pegar o ID do subject (sub) do token
                    const userId = payload.sub || payload.userId || payload.user_id || payload.id;
                    if (userId) {
                        console.log('✅ ID do usuário extraído do token:', userId);
                        return String(userId); // Converte para string para comparação
                    }
                }
            }
            
            // MÉTODO ALTERNATIVO: Dados salvos no localStorage
            ...
            if (userData) {
                ...
                if (userId) {
                    console.log('✅ ID do usuário encontrado em localStorage:', userId);
                    return String(userId);
                }
            }
            
            // MÉTODO DIRETO: ID salvo diretamente
            const directUserId = localStorage.getItem('userId') || 
                               localStorage.getItem('currentUserId') || 
                               localStorage.getItem('user_id');
                               
            if (directUserId) {
                console.log('✅ ID do usuário encontrado diretamente:', directUserId);
                ...
                <div className="details-header">
                    ...
                    {process.env.NODE_ENV === 'development' && (
                        <div style={{
                            ...
                            <div>Usuario atual: {currentUser?.id || 'Carregando...'}</div>
                            <div>Usuario nome: {currentUser?.fullName || 'Carregando...'}</div>
                            <div>Anunciante ID: {advertisement?.advertiser?.id}</div>
                            <div>Advertiser ID Alt: {advertisement?.advertiserId}</div>
                            <div>Nome anunciante: {getAdvertiserName()}</div>
                            <div>WhatsApp: {getWhatsAppNumber() || 'N/A'}</div>
                            <div>É dono: {isOwner() ? 'SIM' : 'NÃO'}</div>
                            <div>User loading: {userLoading ? 'SIM' : 'NÃO'}</div>
                        </div>
                    )}
                        ...
                        <div className="image-gallery">
                            <div className="main-image">
                                <img 
                                    ...
                                    onError={(e) => {
                                        e.target.src = 'https://via.placeholder.com/600x400/f0f0f0/666?text=Sem+Imagem';
                                    }}
                                />
                        ...
                        <div className="contact-section">
                            {hasWhatsApp() ? (
                                ...
                                <div className="no-whatsapp-info">
                                    <p style={{color: '#666', fontSize: '14px', textAlign: 'center', padding: '10px'}}>
                                        📵 WhatsApp não informado pelo anunciante
                                    </p>
                                </div>
                            )}
